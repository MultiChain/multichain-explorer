...
MultiChain Explorer (Explorer) is a fork of the popular [Abe](https://github.com/bitcoin-abe/bitcoin-abe) project to add support for MultiChain blockchains with assets and permissions.  MultiChain nodes must be online for all Explorer functions to work.
...
__The rest of this document assumes your blockchain is named ````chain1````. If not, please substitute accordingly.__
...
2. Delete the explorer database file ````chain1.explorer.sqlite```` (path set in ````connect-args```` parameter in ````chain1.conf````)
